# ClipboardSharePlugin

[Sailfish](https://sailfishos.org) transfer engine plugin for copying text and url into clipboard.

Fork of the original [ClipboardSharePlugin](https://github.com/Maledictus/harbour-clipboardshareplugin) to support Sailfish >= 4.4.

## Install

Install via [Chum](https://github.com/sailfishos-chum/main) or [OpenRepos](https://openrepos.net/content/ilpianista/papocchio).

## Donate

The SailfishOS Community Team is on Liberapay:

[![Liberapay receiving](https://img.shields.io/liberapay/receives/SailfishOScommunityTeam?logo=liberapay&label=SailfishOSCommunity)](https://liberapay.com/SailfishOScommunityTeam)

[![Liberapay receiving](https://img.shields.io/liberapay/receives/ilpianista?logo=liberapay&label=ilpianista)](https://liberapay.com/ilpianista)

## License

MIT
